...
final class WeeklyApp: NSObject, NSApplicationDelegate, NSMenuDelegate, NSWindowDelegate {
    ...
    private func createMenu() {
        ...
        menu.delegate = self
        for item in [titleItem, usedItem, resetItem, updatedItem, errorItem] {
    ...
    private func createWindow() {
        window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 410, height: 320), styleMask: [.titled, .closable], backing: .buffered, defer: false)
        window.title = "Codex 주간 잔량"
        window.isReleasedWhenClosed = false
        window.delegate = self
        window.center()
        let caption = NSTextField(labelWithString: "Codex 주간 사용 한도")
        caption.font = .systemFont(ofSize: 14, weight: .medium)
        caption.textColor = .secondaryLabelColor
        bigLabel.font = .monospacedDigitSystemFont(ofSize: 38, weight: .semibold)
        ...
        buttons.spacing = 8
        let stack = NSStackView(views: [caption, bigLabel, progress, resetLabel, updateLabel, messageLabel, buttons])
    ...
    private func render() {
        let expired = usage?.isExpired() ?? false
        let stale = lastError != nil || (usage.map { Date().timeIntervalSince($0.fetchedAt) > 150 } ?? false)
        if let usage, !expired {
            ...
            bigLabel.stringValue = "\(usage.remainingText) 남음"
        ...
        statusItem.button?.toolTip = [titleItem.title, resetItem.title, updatedItem.title, lastError].compactMap { $0 }.joined(separator: "\n")
