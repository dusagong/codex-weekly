...
Codex Weekly는 macOS 메뉴 막대에 Codex 주간 사용 한도와 초기화까지 남은 시간의 비율을 함께 표시하는 네이티브 앱입니다. 메뉴를 열지 않아도 두 비율을 한눈에 비교하고, 상세 창에서 초기화 시각과 갱신 상태를 확인할 수 있습니다.
...
- **항상 보이는 두 비율** — 메뉴 막대에 차트 아이콘과 `65% · ⏳17%`처럼 남은 한도·시간 비율을 함께 표시합니다.
...
메뉴 막대의 차트 아이콘 옆에는 두 비율이 항상 표시됩니다. **`65% · ⏳17%`는 주간 사용 한도가 65%, 주간 시간이 17% 남았다는 뜻입니다.** 앞의 숫자는 정확한 토큰 개수가 아닌 남은 사용 한도의 비율이고, 모래시계 뒤의 숫자는 계정의 다음 초기화까지 남은 시간의 비율입니다. 클릭하지 않아도 두 값을 비교할 수 있으며, 아이콘을 클릭하면 상세 정보와 다음 메뉴를 사용할 수 있습니다.
...
초기화 시각은 Mac의 현지 시간대로 표시됩니다. 조회에 실패하거나 마지막 조회 값이 오래되면 사용 한도 비율 옆에 `?`가 표시됩니다. 초기화 시각을 모르면 시간 비율 자리에 대시가 표시됩니다. 조회된 초기화 시각이 지나면 새 조회가 성공할 때까지 이전 한도 대신 대시 또는 조회 중 표시인 말줄임표를 표시하고, 시간은 `0%`로 표시합니다. 조회할 수 없는 값을 잔량 0으로 처리하지 않습니다.
